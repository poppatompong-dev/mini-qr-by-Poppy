...
export function validateShareManifestPayload(
  payload: unknown,
  limits: ShareLimits = { maxUploadBytes: MAX_UPLOAD_BYTES, maxFilesPerShare: MAX_FILES_PER_SHARE }
  ...
  if (files.length > limits.maxFilesPerShare) return { ok: false, code: 'TOO_MANY_FILES' }

  let totalSize = 0
  for (const file of files) {
    if (!file || typeof file !== 'object') return { ok: false, code: 'INVALID_FILE' }
    const item = file as Partial<ShareManifestItem>
    if (typeof item.safeName !== 'string' || typeof item.storageName !== 'string')
      return { ok: false, code: 'INVALID_FILE' }
    if (typeof item.size !== 'number' || item.size < 0) return { ok: false, code: 'INVALID_FILE' }
    if (item.safeName.length > MAX_FILENAME_LENGTH)
      return { ok: false, code: 'FILENAME_TOO_LONG', filename: item.safeName }
    if (BLOCKED_FILE_EXTENSIONS.has(getFileExtension(item.safeName)))
      return { ok: false, code: 'BLOCKED_FILE_TYPE', filename: item.safeName }
    if (!/^file_\d+(\.[a-z0-9]+)?$/i.test(item.storageName))
      return { ok: false, code: 'INVALID_STORAGE_NAME', filename: item.storageName }
    totalSize += item.size
  }

  if (totalSize > limits.maxUploadBytes) return { ok: false, code: 'TOTAL_SIZE_EXCEEDED' }
  return { ok: true }
}
